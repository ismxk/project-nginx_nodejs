...
3. 安装完成后，在命令行（PowerShell 或 CMD）中检查 Node.js 是否成功安装：
...
2. 安装 express 模块（一个简单的 Web 框架）：

```bash
npm install express
```

3. 创建一个简单的 Node.js 服务器 server.js：
...
## 配置 Nginx 作为反向代理
### 修改 Nginx 配置文件
1. 找到 Nginx 配置文件 nginx.conf，该文件位于 Nginx 解压目录下的 conf 文件夹中，路径为：
...
2. 修改配置文件，让 Nginx 将请求代理到 Node.js 应用（假设 Node.js 应用运行在 http://localhost:3000）：
...
1. 安装和配置 Node.js：构建了一个简单的 Node.js 应用，监听端口 3000 并返回一个简单的响应。

2. 安装和配置 Nginx（Windows 版）：将 Nginx 配置为反向代理服务器，代理客户端请求到 Node.js 应用。

3. 验证项目：通过访问 http://localhost，确保 Nginx 成功将请求转发给 Node.js 应用，并且可以看到正确的响应。
...
3. 终止 Nginx 后，可以使用 netstat 或 tasklist 确认 80 端口是否仍然被占用：
